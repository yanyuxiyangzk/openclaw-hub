...
const fetchData = async () => {
  ...
  loading.value = true
  try {
    const res = await dashboardApi.getAgentsTasksChart(days.value)
    if (res.data.code !== 0) return
    const { labels, datasets } = res.data.data
    hasNoData.value = datasets.length === 0 || datasets.every(d => d.data.every((n: number) => n === 0))
    chartInstance.setOption({
      tooltip: { trigger: 'axis' },
      legend: { data: datasets.map(d => d.agent_name), bottom: 0 },
